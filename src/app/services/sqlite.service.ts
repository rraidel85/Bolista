...
export class SQLiteService {
  ...
  async saveMany(
    ...
    list.map((obj) => {
      ...
      const newKeys = objKeys.filter((x) => !keys.includes(x));
      if (newKeys.length !== 0) keys.push(...newKeys);
    });
    const insertValues = list.map((obj) => {
    ...
    const updateValues = keys.map((key) => {
      ...
      let statement=`${key}= CASE ${keys[0]} `
      const values = list.map((obj) => {
        if(obj[key]){
          if(typeof obj[key]==='number' ){
            return `WHEN '${obj[keys[0]]}' THEN ${key}+${obj[key]}\n`;
          }else{
            return `WHEN '${obj[keys[0]]}' THEN ${obj[key]}\n`;
          }
        }
        return;
      }).filter(x=>x!=='');

      statement=statement+values.join(' ')+` ELSE ${key} END`
      return statement
    });
    updateValues.shift()
    stmt =  `INSERT INTO ${table} (${keys.join(', ')})
              VALUES
              ${insertValues.join(',\n')}
              ON CONFLICT(${keys[0]}) DO UPDATE SET
                 ${updateValues.join(',\n')}
              `;
    // if (!isUpdate) {
      // stmt = `INSERT INTO ${table} (${keys.join(', ')})
    ...
    const ret = await mDb.execute(stmt);
    /* if (ret.changes!.changes != 1) {
      return Promise.reject(`save: insert changes != 1`);
    } */
    return;
